...
def modelMerge(models,activation='sigmoid'):
    ...
    out = tf.keras.layers.Dense(1,activation=activation)(concatenated)    
    modelOut = tf.keras.models.Model(inputs=inputs,outputs=out)
    return modelOut
...
#                    print(tmpShape)
    ...
    newModel.add(tf.keras.layers.Reshape(tuple(tmpShape), input_shape=(dataFeaLength,)))
    newModel.add(model)
    return newModel
...
#        model.summary()
    ...
    out = tf.keras.layers.Dense(outDim,activation=activation)(concatenated)  
